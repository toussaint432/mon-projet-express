...
Créer et travailler sur dev:
git checkout -b dev
git add .
git commit -m "Test CI/CD sur dev"
git push origin dev
...
on:
  push:
    branches:
      - dev        # pipeline s'exécute uniquement sur dev pour tester
      # - main     # décommenter pour production

      git checkout main     #se positionner dans mains
    git pull origin main    # mettre à jours
